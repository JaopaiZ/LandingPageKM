...
type DurationKey = 'd7' | 'm1' | 'm2' | 'm3' | 'm6' | 'y1';
...
type PackageItem = {
  ...
  tags: string[];
  highlights: string[];
  prices: Partial<Record<DurationKey, number>>;
...
const packages: PackageItem[] = [
  ...
  },
  {
    ...
    prices: { m1: 39, m3: 109, m6: 199, y1: 369 },
    bestValue: 'y1'
  },
  {
    id: 'prime-video',
    name: 'Prime Video 4K',
    orderLabel: 'Prime Video 4K',
    tags: ['4K', 'Premium'],
    highlights: ['หนังและซีรีย์อัปเดตสม่ำเสมอ', 'รองรับ 4K', 'เหมาะกับผู้ชมหลากหลายแนว'],
    prices: { m1: 69, m3: 189, m6: 369, y1: 699 },
    bestValue: 'y1'
  },
  {
    id: 'iqiyi',
    name: 'iQIYI 爱奇艺 4K',
    orderLabel: 'iQIYI 爱奇艺 4K',
    tags: ['4K', 'Premium'],
    highlights: ['ซีรีย์จีน/เอเชียยอดนิยม', 'ดูต่อเนื่องไม่มีสะดุด', 'คุณภาพคมชัด'],
    prices: { m1: 59, m3: 169, m6: 329, y1: 649 },
    bestValue: 'y1'
  },
  {
    id: 'chatgpt-plus',
    name: 'ChatGPT Plus',
    orderLabel: 'ChatGPT Plus',
    tags: ['Premium'],
...
export default function LandingPage() {
  const [selectedDurations, setSelectedDurations] = useState<Record<string, DurationKey>>(
    Object.fromEntries(
      packages.map((item) => {
        const initialDuration = durationOrder.find((key) => item.prices[key] !== undefined) ?? 'm1';
        return [item.id, initialDuration];
      })
    )
  );
  ...
  const handleCopyOrderText = async (item: PackageItem, duration: DurationKey, price: number) => {
    const text = `${item.orderLabel} / ${durationLabels[duration]} / ${price.toLocaleString()} บาท`;
    try {
  ...
  return (
    <main className="pb-24">
      ...
      <section id="packages" className="section-wrap mt-14 scroll-mt-8">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {packages.map((item) => {
            const availableDurations = durationOrder.filter((duration) => item.prices[duration] !== undefined);
            const currentDuration = selectedDurations[item.id];
            const price = item.prices[currentDuration] ?? item.prices[availableDurations[0]] ?? 0;
            ...
            return (
              <article key={item.id} className="rounded-2xl border border-zinc-800 bg-luxury-card p-5 transition hover:-translate-y-1">
                ...
                <div className="mt-4 grid gap-2">
                  <button
                    onClick={() => handleCopyOrderText(item, currentDuration, price)}
                    className="rounded-xl border border-luxury-gold/40 bg-black/30 px-4 py-2.5 text-sm font-semibold text-luxury-softGold transition hover:bg-luxury-gold/10"
                  >
                    คัดลอกข้อความสั่งซื้อ
                  </button>
                  {copiedId === item.id && <p className="text-center text-xs text-emerald-300">คัดลอกข้อความแล้ว</p>}
                  <a
                    href={LINE_LINK}
                    target="_blank"
                    rel="noreferrer"
                    className="block rounded-xl bg-white/5 px-4 py-2.5 text-center text-sm font-semibold text-luxury-softGold ring-1 ring-luxury-gold/30 transition hover:bg-luxury-gold/15"
                  >
